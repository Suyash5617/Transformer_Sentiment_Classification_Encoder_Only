...
OUPTUT_PATH = "breast-cancer-wisconsin.csv"
...
# Desription :     Read the data into pandas datafram
...
# Desription :     Add the headers to the dataset
# Input :          dataset
# Output :         Upadate dataset
# Author :         Rushikesh Hanumant Gade
# Data  :          09/08/2025 
...
def add_headers(dataset, headers):
    ...
    dataset.coloumns = headers
    return dataset
##########################################################
# Function name:   data_file_to_cvs
# Input :          Nothing
...
def data_file_to_csv():
    """Convert row.data file to CSV with header"""
    dataset = read_data(INPUT_PATH)
    dataset = add_headers(dataset, HEADERS)
    dataset.to_csv(OUPTUT_PATH, index=False)
    print("File saved...!")
##########################################################
# Function name:   handel_missing_values
# Desription :     Filter missing values from the dataset
...
def handle_missing_values_with_imputer(df,feature_headers):
    """Conver '?' to NaN and let SimpleImpute handle them inside the Pipeline.
    Keep only numbric column in features.
    """

    # Replace '?' in the whole dataframe
    df = df.replace('?',np.nan)

    # Cast feature to numeric
    df[feature_headers] = df[feature_headers].apply(pd.to_numeric, errors='coerce')
    return df
##########################################################
# Function name:   split_dataset
# Desription :     split the dataset with train_percentage
...
def split_dataset(dataset, train_percentage, feature_headers, target_header, random_state = 42):
    ...
    train_x,test_x,train_y,test_y = train_test_split(dataset[feature_headers],dataset[target_header],train_size=train_percentage,random_state=random_state,stratify=dataset[target_header])

    return train_x,test_x,train_y,test_y
##########################################################
# Function name:   dataset_statistics
# Desription :     Display the statistics
# Author :         Rushikesh Hanumant Gade
# Data  :          09/08/2025 
##########################################################
...
# Desription :     Build a Pipeline
# SimpleInput :    replace missing with median     
...
def build_pipeline():
    ...
    pipe = Pipeline(steps=[
        ("imputer",SimpleImputer(strategy="median")),("rf",RandomForestClassifier(n_estimators = 300,random_state= 42,n_jobs = -1,class_weight=None))])
    return pipe
##########################################################
# Function name:   train_pipeline
# Desription :     Train a Pipeline
# Author :         Rushikesh Hanumant Gade
# Data  :          09/08/2025
##########################################################
...
# Desription :     Load the trained model
# Author :         Rushikesh Hanumant Gade
# Data  :          09/08/2025
##########################################################
...
# Desription :     Display Confusion Matrix
# Author :         Rushikesh Hanumant Gade
# Data  :          09/08/2025
##########################################################
def plot_confusion_matrix_matshow(y_true, y_pred, title= "Confusion Matrix"):
    ...
    plt.show()
##########################################################
# Function name:   plot_feature_importanance
# Desription :     Display the feature importance
# Author :         Rushikesh Hanumant Gade
# Data  :          09/08/2025
##########################################################
def plot_feature_importance(model,feature_names, title = "Feature Importances(Random Forest)" ):
    if hasattr(model,"named_steps") and "rf" in model.named_steps:
        ...
        print("Feature importance not availabe for this model.")
        return     
...
##########################################################
# Function name:   main
# Desription :     Main function from where execution starts
# Author :         Rushikesh Hanumant Gade
# Data  :          09/08/2025
##########################################################
def main():
    ...
    # 2) Load CSV
    dataset = pd.read_csv(OUPTUT_PATH)
    
    # 3) Basic stats
    dataset_statistics(dataset)

    # 4) Prepare feature/target
    feature_headers = HEADERS[1 : -1]   #drop CodeNumber, keep all features
    target_header = HEADERS[-1]        # CancerType(2 = bening, 4 = malignant)

    # 5) Handle '?' and corece to numeric; imputation will happen inside Pipeline
    dataset = handle_missing_values_with_imputer(dataset, feature_headers)
     
    # 6) Split
    train_x, train_y, test_x,test_y = split_dataset(dataset, 0.7, feature_headers, target_header) 
    ...
    print("Classifion Report:\n",classification_report(test_y,predictions))
    print("Confusion Matrix:\n",confusion_matrix(test_y, predictions))

    #Feature importances (tree-based)
    plot_feature_importance(trained_model, feature_headers,title="Feature Importance(RF)")
